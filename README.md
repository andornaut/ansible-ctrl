...
| [router.yml](router.yml)                 | `routers`, then `faramir_controller`  | [router](roles/router/README.md), then its `kva-sample` entry point (`tasks_from`)                                                           | Connectivity and resolver health checks on the pfSense routers, and the kernel address space sampler on the controller |
