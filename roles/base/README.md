...
| The `unwanted` set ([vars/main.yml](./vars/main.yml)) is what Ubuntu installs by default and no host here has hardware or a role for | `kdump-tools` (crash dumps nobody reads, and it reserves memory at boot through GRUB's `crashkernel=`, returned at the next reboot), `modemmanager` (no modems), `power-profiles-daemon` and `switcheroo-control` (no laptop power profiles or dual-GPU switching), `speech-dispatcher`, `language-selector-gnome`, `open-vm-tools` (VMware guest tools on bare metal), `pollinate` (one-shot entropy seeding) and `sssd` (no directory service). In `vars/`, so a host cannot opt out of a purge set |
...
| `/tmp` is aged at `base_tmp_max_age` ([tasks/systemd.yml](./tasks/systemd.yml)) | `/etc/tmpfiles.d/tmp.conf` overrides the shipped 30d, which is long enough for a desktop's `/tmp` to reach several GB before anything is collected. `systemd-tmpfiles-clean.timer` applies it daily and only removes a directory once its contents have aged out |
