...
| What | Detail |
| --- | --- |
| Accounts are closed to each other | `HOME_MODE` and `adduser`'s `DIR_MODE` at `0750`, `o-rwx` on each login account's home. Nothing below the home needs converging. Group access is untouched: `USERGROUPS_ENAB` gives each account a private group, and what is shared across accounts is shared by group |
| A session's default modes | `UMASK 002` in `/etc/login.defs`, read by `pam_umask` for login shells and SSH sessions. A default, not a boundary: a process may change it, and systemd units read `UMask=` instead. 002 keeps a file created in a setgid share group-writable, as faramir's tree and the NAS share need |
