...
| Locked encrypted home     | The run fails while `bspwm_user`'s home is encrypted and not mounted, before anything changes: an encrypted home is a mount point that `getent` reports either way, and the desktop entry overrides would land under it. Log in as `bspwm_user`, or run `ecryptfs-mount-private`, and re-run     |
| Build account             | The clones, the builds and a `DESTDIR` staging install run as `bspwm_user`; only the copy of the staged tree into `/usr/local` runs as root                                                                                                                                                      |
| Rebuilt on every run      | `bspwm_projects` build from `HEAD` by default, which is past each project's last tag, and the binaries report only the tagged version, so no installed artifact says whether it is current                                                                                                       |
