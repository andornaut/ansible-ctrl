...
| codex           | Upstream's standalone installer, run as `dev_user`, which puts a self-contained binary at `~/.local/bin/codex` and needs no Node. `CODEX_NON_INTERACTIVE` answers no to the prompts it would otherwise block the run on. It never updates once installed: `codex` updates itself, or re-run the installer                                                                                                                                                                                                                                                                                                                                                             |
