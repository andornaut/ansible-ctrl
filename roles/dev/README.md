...
| Architectures          | `dev_arch` maps the kernel name to Debian's for the Antigravity and Claude apt repositories, the Go archive and `JAVA_HOME`. Cursor and ai-attributions name architectures their own way (`dev_cursor_platform`, `dev_ai_attributions_release_archive`). The Android SDK is x86_64 only, asserted by `android_sdk` |
