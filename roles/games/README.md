...
| Core downloads                            | `get_url`, then `unarchive` only when the archive changed or the core is missing. The buildbot never returns `304`, so every run downloads the archives, but a core reports changed only when its binary changed. Letting `unarchive` download re-extracts and reports every core changed on every run                                                        |
...
| VRR outside RetroArch                     | Not checked by the play. Enable it for the display: a compositor setting under Wayland, or `Option "VariableRefresh"` in `xorg.conf.d` under X11, where it conflicts with `TearFree`                                                                                                                                                                          |
...
| Icon                                 | The one lutris.net serves for the slug, stored under `~/.local/share`: Lutris keeps its copy inside the sandbox, where the host's launcher does not look. Revalidated each run with If-Modified-Since, and not fatal on failure: the desktop entry is installed before it                                                                                                           |
...
| Constraint                           | Detail                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `vars/main.yml` is not host-settable | Role vars outrank `host_vars`. It holds the canonical RetroArch data (`games_retroarch_systems`, `..._required_settings`, `..._core_overrides`, `..._core_options`, `..._controllers`), derived paths, application IDs, flatpak sandbox grants and fact-derived values                                                                                                                                                                                                                                                   |
| The handheld reads `vars/` only      | `syncretroid` reads `vars/main.yml` directly and resolves no inventory: it drops every templated setting, and `profile.yml` supplies the device's own. A per-host setting (`games_retroarch_extra_settings`, the refresh rate, the controller) reaches the desktops only                                                                                                                                                                                                                                                 |
...
| Flatpak overrides are written whole  | `flatpak_override.yml` resets each override, then applies the role's grants, so a manual `flatpak override --user` edit is lost on the next run. BedrockOnLinux's comes from `bedrock.yml`, and Lutris's from `flatpak.yml` under both the flatpak and lutris tags. With `games_gamescope_enabled` on, each launcher's PATH is its manifest's with the gamescope helper directory first. BedrockOnLinux's PATH always ends with `games_bedrock_helper_dir`, and its manifest sets none, so the runtime default stands in |
| No dependencies                      | The role installs `flatpak` and adds the flathub remote for `games_user` itself                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Encrypted home must be mounted       | `getent` returns the home's path whether or not it is mounted, so a run against a locked home would write every per-user file onto the mountpoint, where the next login hides it. `main.yml` checks `<home's parent>/.ecryptfs/<user>`, which is outside the home and present either way, then requires the home among `ansible_facts["mounts"]` and fails the run before any change. Log in as `games_user`, or run `ecryptfs-mount-private`, and re-run                                                                |
