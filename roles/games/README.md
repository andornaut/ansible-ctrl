...
| gamemode  | `/etc/gamemode.ini`, which is the screensaver inhibitor and nothing else, and the polkit rule that lets the daemon's root helpers run for `games_user`                                                                                                                                                     |
...
| The daemon's helpers need a polkit grant, which the tag writes for `games_user`             | `cpugovctl`, `cpucorectl`, `gpuclockctl` and `procsysctl` run as root through `pkexec`, and the shipped policy answers "no" for every account. Without the rule the daemon enters Game Mode and no governor, pinning or split-lock change lands, each refusal logged as `Not authorized`. Granted to that account's local, active session only |
| Under umu the game itself never registers                                                   | The client library the wrapper preloads cannot find `libgamemode.so` inside pressure-vessel's container, one log line per process start. The wrapper outside the container registers instead, so the system-wide settings apply; only the per-process renice of the game's own PID is lost                                                     |
