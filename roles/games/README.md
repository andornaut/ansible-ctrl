...
| The prefix is read out of Battle.net's own Lutris configuration            | `games/battlenet.yml`, so the probe and the teardown name the prefix the client actually uses; the default install path this role enforces is only the fallback. A prefix holding no `_retail_/Wow.exe` is named in a warning and nothing is registered |
