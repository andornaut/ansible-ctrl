...
| gamemode  | `/etc/gamemode.ini`, which is the screensaver inhibitor and nothing else, and membership of the `gamemode` group for `games_gamemode_users`, which is what grants the daemon's root helpers                                                                                                                |
...
| The daemon's helpers run as root, granted by membership of the `gamemode` group             | `cpugovctl`, `cpucorectl`, `gpuclockctl` and `procsysctl` run through `pkexec`. The package ships the polkit rule granting the four to that group and puts nobody in it, so out of the box the daemon enters Game Mode and no governor, pinning or split-lock change lands, each refusal logged as `Not authorized`. The tag adds `games_gamemode_users`. The grant is account-wide: any process of a member can run the four helpers without a prompt, and one that bypasses the daemon is never undone |
| What lands is upstream's `gamemode.ini` defaults, host-wide                                 | `desiredgov=performance` and `disable_splitlock=1` reach every core and `/proc/sys/kernel/split_lock_mitigate` for as long as any client is registered, and are restored when the last one leaves. The daemon holds the values it restores in memory only: one that dies mid-session is replaced by one that takes the game-time values as its baseline and never restores them                                                                                                                          |
| Under Lutris the game itself never registers                                                | Lutris's own GameMode option prefixes the launch with `gamemoderun`, whose client library cannot find `libgamemode.so` inside umu's pressure-vessel container, one log line per process start. `gamemoderun` itself, outside the container, registers instead, so the host-wide settings apply; the per-process passes (ioprio, pinning) skip the game's own PIDs                                                                                                                                        |
...
| A second activation while a launch is in progress waits, then gives up                                | The launcher holds an exclusive lock on the prefix, under the runtime directory, from the click until the game has a window or the wait for one has ended. Without it a second click, the ordinary response to twenty seconds with nothing on screen, tears down the processes the first one has just started. The second click waits up to thirty seconds behind the first, its banner saying so, then reports that it did not start                                                                                                                                                                                                          |
| A click on a session that has no window relaunches it                                                 | Once the lock is released the session belongs to the next click. Battle.net's Agent stays resident after the game quits, so its Lutris instance never exits on its own. A session whose gamescope display shows a mapped window of usable size is running, and the click says so and leaves it alone. One that shows none is torn down and launched again; the launcher that started it is terminated first, so it does not report its Lutris being killed as a failure                                                                                                                                                                        |
| Its notifications are transient                                                                       | None reaches the message list: an entry left there stays until dismissed by hand, and a second run cannot replace it, the id reaching no further than the process that got it. A failure shows a twenty-second banner and is otherwise missed                                                                                                                                                                                                                                                                                                                                                                                                  |
| The banner is re-sent every three seconds for as long as something is waited for, ten minutes at most | Its body names the wait: the lock, the previous session's teardown, the previous Lutris instance's exit, Lutris starting, the Wine session starting, a Proton update, the game's window. Lutris sets `PROTONPATH=GE-Proton`, so the first launch after a GE-Proton release downloads and unpacks it before anything appears, and Battle.net then takes its own time to put a window up. The tarball lands in the sandbox's private `/tmp`, but umu holds a `tmp*` directory under the flatpak's umu cache for exactly that span, so the banner says a Proton update is being installed while one that appeared after the launch began persists |
| The window is read from gamescope's nested X server                                                   | `gamescope-child` exports its display, so the host reads the display out of the game's environment in `/proc` and asks that server for its children, taking one at least 64 pixels on both sides and mapped. The nested server is an Xwayland on an X11 host and a Wayland one alike. A launch with no gamescope puts the window among the desktop's own, where it cannot be told apart, and ends at the prefix's wineserver instead                                                                                                                                                                                                           |
| It logs every decision to `~/.local/state/lutris-launch-game/<slug>.log`                              | A desktop entry's stderr reaches nothing. The lock's state, every process found and signalled, every change of the banner, what the nested display shows and how Lutris exited are written there, rotated at 1 MiB. Read it first when a launch does nothing. `XDG_STATE_HOME` relocates it                                                                                                                                                                                                                                                                                                                                                    |
| A clean Lutris exit does not end the wait                                                             | The client exits 0 whatever became of the launch, and a second instance exits 0 at once having handed its request to the first. The wait ends instead when no sandbox process is left, or at the deadline, which is reported as a failure                                                                                                                                                                                                                                                                                                                                                                                                      |
