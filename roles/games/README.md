...
| A second activation while a launch is in progress does nothing            | The launcher holds an exclusive lock on the prefix, under the runtime directory, for the whole run. Without it a second click, the ordinary response to twenty seconds with nothing on screen, tears down the processes the first one has just started. The second run says so in a notification and exits                                                                                                                                            |
| A click made just after quitting the game waits rather than being refused | The holder keeps the lock as long as the game runs, and releases it only once its Lutris instance has shut down, a few seconds later. The click waits up to thirty seconds for that, showing "Closing the previous session first.", and then launches. A prefix whose session still has a window is running rather than ending, and that click says so and leaves it alone                                                                            |
