...
| The banner is re-sent until the prefix's wineserver is up, ten minutes at most | Lutris sets `PROTONPATH=GE-Proton`, so the first launch after a GE-Proton release downloads and unpacks it before any window appears, minutes during which nothing on screen says a launch is in progress. The tarball lands in the sandbox's private `/tmp`, but umu holds a `tmp*` directory under the flatpak's umu cache for exactly that span, so the banner says a Proton update is being installed while one that appeared after the launch began persists. A clean Lutris exit ends nothing, the client exiting 0 whatever became of the launch; the wait ends when no sandbox process is left, or at the deadline, reported as a failure |
