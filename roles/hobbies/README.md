...
| `/opt/freerouting/freerouting-<version>-<platform>`                           | freerouting release; `/opt/freerouting/current` links to it, and every other `freerouting-*` directory there is removed |
...
| Release architectures | Freerouting publishes a `linux-x64` build only, and the role asserts one exists for the host. ExpressLRS takes the `.deb` named for the Debian architecture in `hobbies_arch`, asserted to exist. Betaflight's Linux portable build is x86_64 only, asserted by `betaflight`                        |
