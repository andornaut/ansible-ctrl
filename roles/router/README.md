...
| Each script keeps a lock file under `/var/run/`, whose mtime is its last run                               | Nothing else records that either script is alive: a healthy run logs nothing by design, and pfSense keeps no cron log and no `cron` syslog facility. `stat` the file to tell a check that is passing from one that stopped running. The lock also keeps two runs apart, which for `router-pingtest` would otherwise have one downing the interface while the other decides whether it answers |
