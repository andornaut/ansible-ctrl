...
- **Remote host:** rtorrent as a `Type=forking` systemd service inside a tmux session on a private socket.
...
| Property | Constraint |
| --- | --- |
| The `result` event prints unclipped, keeping its line breaks | It is the only full copy of the run's report, deferred entries included. Tool calls and narration collapse to one clipped line; a `Bash` line shows the command rather than its description |
...
The `shell` check in [tests/lint.sh](../../tests/lint.sh) ShellChecks these templates, rendering Jinja2 expressions
to placeholders first. Suppress a finding with a `# shellcheck disable=...` comment in the template.
